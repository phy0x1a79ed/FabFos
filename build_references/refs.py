"""Paths, encoding and bake identity for the compiled metabolism reference tables.

The compiled tables are three parquet files that together replace two string-keyed
tables read from a sibling project's scratch tree:

    data/reference/metabolism/vocab.parquet        int-code vocabulary, 5 kinds
    data/reference/metabolism/atom_pairs.parquet   the atom-atom mapping
    data/reference/metabolism/direction.parquet    per-reaction directionality
...
# build_references/refs.py -> the repo root is one level up
REPO = Path(__file__).resolve().parents[1]

# -- inputs: frozen, immutable, pinned by DVC ---------------------------------
RAW_DIR = REPO / "data" / "raw" / "mnxref-4_5"
SRC_ATOM_PAIRS = RAW_DIR / "atom_pairs.parquet"
SRC_DIRECTION = RAW_DIR / "direction.parquet"
SRC_MANIFEST = RAW_DIR / "MANIFEST.json"

# -- outputs ------------------------------------------------------------------
OUT_DIR = REPO / "data" / "reference" / "metabolism"
VOCAB = OUT_DIR / "vocab.parquet"
ATOM_PAIRS = OUT_DIR / "atom_pairs.parquet"
DIRECTION = OUT_DIR / "direction.parquet"
BAKED = (VOCAB, ATOM_PAIRS, DIRECTION)
...
def read_identity(path) -> dict:
    ...
    if raw is None:
        raise ValueError(
            ...
            f"bake_metabolism.py, or it was rewritten by a tool that dropped the "
...
def assert_same_bake(*paths) -> dict:
    ...
    error.
    """
    paths = [Path(p) for p in (paths or BAKED)]
    ...
    if first.get("bake_version") != BAKE_VERSION:
        raise ValueError(
            f"bake_version {first.get('bake_version')} but this refs.py speaks "
            f"{BAKE_VERSION}")
...
def load_vocab(path=None) -> Vocab:
    return Vocab(pd.read_parquet(path or VOCAB))


def load_atom_pairs(element: str | None = None, vocab: Vocab | None = None,
...
def load_direction(path=None) -> pd.DataFrame:
    return pd.read_parquet(path or DIRECTION)


def ratio_by_code(vocab: Vocab, direction: pd.DataFrame | None = None) -> np.ndarray:
    ...
    """
    direction = load_direction() if direction is None else direction
...
def compile_atom_graph(element: str, weights: dict, *, ratios: bool = True,
                       use_confidence: bool = False, vocab: Vocab | None = None,
                       pairs: pd.DataFrame | None = None,
                       ratio_lut: np.ndarray | None = None,
                       meta: dict | None = None):
