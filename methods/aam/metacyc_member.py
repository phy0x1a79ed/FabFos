...
def _default_paths():
    ...
    return (data / "references/metacyc26_flatfiles/atom-mappings-smiles.dat",
            data / "references/metanetx/reac_xref.tsv")


def main():
    smi_default, xref_default = _default_paths()
    ap = argparse.ArgumentParser(description=__doc__,
                                 formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--smiles-dat", type=Path, default=smi_default)
    ap.add_argument("--reac-xref", type=Path, default=xref_default)
    ap.add_argument("--out", type=Path,
                    default=Path(__file__).resolve().parent / "metacyc_aam.tsv")
    a = ap.parse_args()
