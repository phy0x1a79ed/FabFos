...
Anchor = the pgdb LEFT/RIGHT slots, which are self-consistent with the pgdb's own
...
EcoCyc reactions carry no dedicated MNXref prefix; they join through their shared
MetaCyc reaction UNIQUE-IDs (the same metacyc.reaction map), which is why EcoCyc
coverage is lower than MetaCyc's.

Standalone: `python -m curated --out <parquet>` (run under an env with pandas).
"""
...
def read_source(pgdb_path: Path, source: str, id2mnxr, sides, cmap, col3):
    """One row per (source reaction) that carries a REACTION-DIRECTION and an MNXR.

    Read-only sqlite: the facade's own Pgdb() opens read-write and runs DDL,
    which must never touch a shared staging file.
    """
    con = sqlite3.connect(f"file:{pgdb_path}?mode=ro", uri=True)
    rows = []
    for key, js in con.execute("SELECT key, json FROM reactions_dat"):
        j = json.loads(js)
        d = j.get("REACTION-DIRECTION")
        if not d:
            continue
        ...
        # the same flip test against the same MNXR sides that the pgdb LEFT/RIGHT
        # got. Both are MetaCyc orientation, so the verdicts should match; a
...
def build(metacyc_pgdb, ecocyc_pgdb, reac_xref, reac_prop, chem_xref):
    ...
    parts = [
        read_source(metacyc_pgdb, "metacyc", id2mnxr, sides, cmap, col3),
        read_source(ecocyc_pgdb, "ecocyc", id2mnxr, sides, cmap, col3),
    ]
    per_rxn = pd.concat(parts, ignore_index=True)
...
def main():
    ...
    ap.add_argument("--metacyc-pgdb", required=True)
    ap.add_argument("--ecocyc-pgdb", required=True)
    ...
    a = ap.parse_args()
    per_rxn, per_mnxr = build(Path(a.metacyc_pgdb), Path(a.ecocyc_pgdb),
                              Path(a.reac_xref), Path(a.reac_prop), Path(a.chem_xref))
