...
file = (total testable axes) x (draws per size) -- read the axis count from
`canon.AXES_N`, never from here. The PREVIOUS canonical null is one axis short of
that: the carbon axis the stale evidence weights had been silently zeroing, which
became testable when the weights were repointed at the CLEAN fosmid basis
(canon.FOSMID_BASIS). So a merged file landing one axis-worth of rows low does not
mean "close enough" -- it means the coherent weights did not take, and the whole
chain is suspect.
