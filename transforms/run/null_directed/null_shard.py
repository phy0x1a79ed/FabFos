"""Self-contained directed-null driver for one (element, draw-size) shard.

VENDORED 2026-07-20 from `fig-model/main/fabfos/directed/fir/fir_null.py`, verbatim
apart from this header. It is copied rather than imported because it is
deliberately canon-free and fully `--stage-dir` parametrised -- that is what lets
it run on a cluster with none of the incumbent tree present -- and because
`transforms/run/null_directed/stage_null.py` is its missing producer, so the two
belong together. The original stays where it is; if it changes, re-vendor rather
than diverge.

Its `--resume` is a ROW-COUNT check only (expected = testable axes x draws), not
a content or engine-version check. Row count is very nearly invariant under a
universe change, so `--resume` will happily skip every shard of a stale run and
leave the old output in place. Clear the output directory explicitly (named
files) whenever the engine, the base graphs or the universe move.
